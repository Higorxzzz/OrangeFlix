...
serve(async (req) => {
  ...
  try {
    ...
    // Only consider itemId if there are more parts after the function name
    // Path structure: /functions/v1/admin-items or /functions/v1/admin-items/{itemId}
    const functionNameIndex = pathParts.findIndex(part => part === 'admin-items');
    const itemId = functionNameIndex >= 0 && pathParts.length > functionNameIndex + 1 
